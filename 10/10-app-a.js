        ...
        if(error){
            ...
            for(let i=0; i<dataObject.records.length; i++){
                if(dataObject.records[i].id === request.data.id){
                    dataObject.records[i].status = !dataObject.records[i].status ;
                    found = 1;
                }
            }
            if(found < 0){
                console.log('Item Created.');
                write(response, 404,'ERROR:' + error)
            }
            ...
            writeFile('./todo.json', dataString, function (error){
                if(error){
                    ...
                    console.log('status chandged successfuly.');
                    write(response, 200, JSON.stringify({status:'status chandged.', id: request.id}))
                }
            });
        }
    });
